 ...
 * 5. The seller recieves the released funds
 * 6. Incase of a dispute arises (Should be before the buyer releases funds in 5) the buyer and seller identify an arbiter(escrow) to help in resolving a conflict incase
 * the conflict is to be escaleted to another party.
 * 7. The arbiter deternines who is in the wrong and either refunds the buyer or releases the funds to the seller.
 * 8. The transaction is considered closed.
 */
...
//function to get number of transactions of an address
    ...
    function getSpecificTransaction(
        ...
        uint switcher,
        uint ID
    ) external view returns (address, address, address, uint, bytes32, uint, bytes32) {
        bytes32 status;
        EscrowStruct memory currentEscrow;
        if (switcher == 0) {
            ...
            currentEscrow = buyerDatabase[sellerDatabase[inputAddress][ID].buyer][
                sellerDatabase[inputAddress][ID].buyer_nounce
            ];
            status = checkStatus(
                currentEscrow.buyer,
                sellerDatabase[inputAddress][ID].buyer_nounce
            ...
            currentEscrow = buyerDatabase[escrowDatabase[inputAddress][ID].buyer][
                escrowDatabase[inputAddress][ID].buyer_nounce
            ];
            status = checkStatus(
                currentEscrow.buyer,
                escrowDatabase[inputAddress][ID].buyer_nounce
        ...
        return (
            ...
            currentEscrow.escrow_agent,
            currentEscrow.amount,
            status,
            currentEscrow.escrow_fee,
            currentEscrow.notes
        );
    }
//buyer history
    function buyerHistory(
        ...
        uint startID,
        uint numToLoad
    ) external view returns (address[] memory, address[] memory, uint[] memory, bytes32[] memory) {
        uint length;
        if (buyerDatabase[buyerAddress].length < numToLoad)
            length = buyerDatabase[buyerAddress].length;
        else length = numToLoad;
...
//seller history
    function SellerHistory(
        ...
        for (uint i = 0; i < numToLoad; i++) {
            if (i >= sellerDatabase[inputAddress].length) break;
            buyers[i] = sellerDatabase[inputAddress][startID + i].buyer;
            escrows[i] = buyerDatabase[buyers[i]][
                sellerDatabase[inputAddress][startID + i].buyer_nounce
            ].escrow_agent;
            amounts[i] = buyerDatabase[buyers[i]][
                sellerDatabase[inputAddress][startID + i].buyer_nounce
            ].amount;
...
//escrow History
    function escrowHistory(
        ...
        for (uint i = 0; i < numToLoad; i++) {
            if (i >= escrowDatabase[inputAddress].length) break;
            buyers[i] = escrowDatabase[inputAddress][startID + i].buyer;
            sellers[i] = buyerDatabase[buyers[i]][
                escrowDatabase[inputAddress][startID + i].buyer_nounce
            ].seller;
            amounts[i] = buyerDatabase[buyers[i]][
                escrowDatabase[inputAddress][startID + i].buyer_nounce
            ].amount;
...
//function to checkstatus
    ...
    function buyerFundRelease(uint ID) public {
        require(
            ID < buyerDatabase[msg.sender].length &&
    ...
    function EscrowEscalation(uint switcher, uint ID) external {
        ...
        require(
            buyerDatabase[buyerAddress][buyerID].escrow_intervention == false &&
                buyerDatabase[buyerAddress][buyerID].release_approval == false &&
                buyerDatabase[buyerAddress][buyerID].refund_approval == false
        );
    ...
    function WithdrawFunds() public {
        ...
        Funds[msg.sender] = 0;
        if (!msg.sender.send(amount)) Funds[msg.sender] = amount;
    }
